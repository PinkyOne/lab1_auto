...
//FILE  *out;
 ...
 VOZD:
  ...
  if(R * (*T)/9.81 > 0)
	(*FFX)=(*GG)*sqrt(R * (*T)/9.81)/XM/ (*PFX)/QL;
  else
	(*FFX)=(*GG)/XM/ (*PFX)/QL;
    ...
    *QTF=(AIK-AIH)/(HU* (*PDF)-15.78*(MM-AIKG)+14.78*(AIK-AIH));
    ...
    else
    {
      ...
      *ALFAF=((*GG)- (*GTS))/((*GTS)+ (*GTF))/14.78;
	  ...
	  if((XK +1)/XK *XKF /(XKF +1)*R /RF * (*T) /(*TF)  > 0)
		ZLF=(1+QTSM)/(1+QTSM+ *QTF)*ZFX*sqrt((XK +1)/XK *XKF /(XKF +1)*R /RF * (*T) /(*TF) );
	  else
	  ...
	  if(ZLF *ZLF -1 > 0)
		XLF=ZLF-sqrt(ZLF *ZLF -1);
	  else
		XLF=ZLF;
      QLAM(&XLF,&XKF,&QL);
	  if((*TF) *RF /9.81 > 0)
		*PF=((*GG)+ (*GTF))*sqrt((*TF) *RF /9.81)/XMF/ (*FFX)/QL;
	  else
...
{ F_real T1,R;
 T1=(*T)/100.0;
 R=(0.24242+T1*(-0.24746e-2+T1*(0.52835e-3+T1*(-0.24661e-4+T1*0.3869e-6))))/(1.0-exp((-1)*(T1-10.101)*(T1-10.101)));
 *ITResult=(*T)*R;
 return 1;
}
 ...
 *R=( (1.0-1.0/(*A)) * 29.27 + (1.0/(*A)/14.78+1.0/(*A))*29.35 ) / (1.0+1.0/(*A)/14.78);
 ...
 if((2./ ((*XK) +1.0)>0) && ((*XK) * pow( 2./ ((*XK) +1.0),((*XK) +1.0)/((*XK) -1.0) ) > 0))
	(*XM)=sqrt( (*XK) * pow( 2./ ((*XK) +1.0),((*XK) +1.0)/((*XK) -1.0) ) );
 else
	(*XM)=1;
 return 1;
}
...
int QLAM(F_real *XL,
	 ...
	 F_real *RES)
{
  if((1- ((*XK) -1)/((*XK) +1)*(*XL) *(*XL)  )>0)
	*RES= (*XL)*pow( fabs(((*XK) +1)/2),1/((*XK) -1) ) * pow( (1- ((*XK) -1)/((*XK) +1)*(*XL) *(*XL)  ) , 1/((*XK) -1) );
  else
	*RES= (*XL)*pow( fabs(((*XK) +1)/2),1/((*XK) -1) );
  return 1;
}
